...
public class ResultsTable implements Serializable {
	...
	private Hashtable<Integer, ArrayList<String>> stringColumns;
	...
	public void setValue(int column, int row, String value) {
		...
		if (stringColumns==null)
			stringColumns = new Hashtable<Integer, ArrayList<String>>();
		ArrayList<String> stringColumn = (ArrayList<String>)stringColumns.get(new Integer(column));
		if (stringColumn==null) {
			stringColumn = new ArrayList<String>();
	...
	public Variable[] getColumnAsVariables(String column) {
		...
		for (int row=0; row<size(); row++) {
			...
			if (Double.isNaN(value) && stringColumns!=null) {
				ArrayList<String> stringColumn = (ArrayList<String>) stringColumns.get(new Integer(col));
	...
	public synchronized void deleteRow(int rowIndex) {
		...
		for (int col=0; col<=lastColumn; col++) {
			if (columns[col]!=null) {
				...
				ArrayList<String> stringColumn = stringColumns!=null?(ArrayList<String>) stringColumns.get(new Integer(col)):null;
	...
	public void loadFromString(String text) throws IOException {
		...
		if (text.startsWith("Error:"))
			throw new IOException("Error loading RT");
		...
		int type=getTableType(lines, firstRow, cellSeparator);
		...
		for (int i=firstRow; i<lines.length; i++) {
			...
			for (int j=firstColumn; j<items.length; j++) {
				if (j==labelsIndex&&labels)
					addLabel(headings[labelsIndex], items[labelsIndex]);
					...
					if (Double.isNaN(value)) {
						...
						addValue(headings[j], item);
					} else
						addValue(headings[j], value);
				}
			}
		}
	}

	private static int getTableType(String[] lines, int firstRow, String cellSeparator) {
	...
	private void writeObject(ObjectOutputStream stream)
		...
		stream.writeUTF(convertToString());
